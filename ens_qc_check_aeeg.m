...
if currSubj.sampling_frequency < 64
    everythingOK = false;
    problem{1} = sprintf('Too low sampling frequency (%1.0f) for acquisition type (aeeg)', currSubj.sampling_frequency);
    problems = cat(1, problems, problem);
end

currSubj.recording_length_in_mins = hdr.nSamples./hdr.Fs./60;
if currSubj.recording_length_in_mins < 1440
    everythingOK = false;
    problem{1} = sprintf('Too low recording length (%1.0f mins) for acquisition type (aeeg)', currSubj.recording_length_in_mins);
    problems = cat(1, problems, problem);
end
...
% 
% % flatline
% updateWaitbar = waitbarParfor(length(dataCut.trial)*size(dataCut.trial{1}), 'Analyzing flatlines...');
...
% badJump = sel;
badKurt = find(any(kurt > 7));
badVar = find(any(var > 1500));
badMax = find(any(maxval > 200));

currSubj.n_bad_trials = length(unique( [ badKurt, badVar, badMax] ));
